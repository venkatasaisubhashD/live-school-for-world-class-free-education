...
public class ListFilesServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        out.println("<input type='button' value='Go Back' onclick='window.history.back()' style='color: white; background-color: black; border: none; padding: 6px 12px; text-align: center; text-decoration: none; display: inline-block; font-size: 12px; margin-top: 20px; cursor: pointer; border-radius: 3px;'>");
        File file = new File(filePath);
        if (file.exists() && file.isDirectory()) {
        	out.println("<h2 style='display:inline;'>List of Files and Folders:</h2>");
        	out.println("<ul>");
            ...
            if (files != null) {
                for (File f : files) {
                    ...
                    if (f.isDirectory()) {
                        out.println("<li><a href='ListFilesServlet?filepath=" + encodedPath + "'>" + icon + f.getName() + "</a></li>");
                    } else {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        response.sendRedirect("ListFilesServlet?filepath=" + URLEncoder.encode(filePath, "UTF-8"));
    }
